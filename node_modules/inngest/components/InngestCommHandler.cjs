...
var InngestCommHandler = class {
	...
	*/
	...
	async handleSyncRequest({ timer, actions, fn, asyncMode, args }) {
		...
		const runId = (0, ulid.ulid)();
		const event = await this.createHttpEvent(actions, fn);
		const exeVersion = require_InngestExecution.PREFERRED_EXECUTION_VERSION;
			...
			if (method === "POST" || forceExecution) {
				...
				if (forceExecution) {
					...
					body = {
						...
						events: [],
						steps: {},
						version: require_InngestExecution.PREFERRED_EXECUTION_VERSION,
						ctx: {
							attempt: 0,
					...
					fn = this.fns[fnId];
					stepId = await actions.queryStringWithDefaults("processing run request", require_consts.queryKeys.StepId) || null;
				}
				...
				[require_consts.ExecutionVersion.V1]: ({ event, events, steps, ctx, version: version$2 }) => {
					...
					return {
						...
						partialOptions: {
							client: this.client,
							runId: ctx?.run_id || "",
							stepMode: fn.fn["shouldAsyncCheckpoint"](requestedRunStep, ctx?.fn_id, Boolean(ctx?.disable_immediate_execution)) ? require_types.StepMode.AsyncCheckpointing : require_types.StepMode.Async,
							data: {
								event,
								events,
								runId: ctx?.run_id || "",
