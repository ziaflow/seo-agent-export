...
var chunk7QC3MHP7_cjs = require('./chunk-7QC3MHP7.cjs');
var chunkABJOUEVA_cjs = require('./chunk-ABJOUEVA.cjs');
var chunk2JUDZXVI_cjs = require('./chunk-2JUDZXVI.cjs');
var chunkRE7SRMBE_cjs = require('./chunk-RE7SRMBE.cjs');
var chunkREVAU76X_cjs = require('./chunk-REVAU76X.cjs');
...
var chunk7ZXDZXGD_cjs = require('./chunk-7ZXDZXGD.cjs');
...
];constructor(options){this.detectionTypes=options.detectionTypes||_PIIDetector.DEFAULT_DETECTION_TYPES;this.threshold=options.threshold??0.6;this.strategy=options.strategy||"redact";this.redactionMethod=options.redactionMethod||"mask";this.includeDetections=options.includeDetections??false;this.preserveFormat=options.preserveFormat??true;this.detectionAgent=new exports.Agent({name:"pii-detector",instructions:options.instructions||this.createDefaultInstructions(),model:options.model,options:{tracingPolicy:{internal:15/* ALL */}}});}async processInput(args){try{const{messages,abort,tracingContext}=args;if(messages.length===0){return messages;}const processedMessages=[];for(const message of messages){const textContent=this.extractTextContent(message);if(!textContent.trim()){processedMessages.push(message);continue;}const detectionResult=await this.detectPII(textContent,tracingContext);if(this.isPIIFlagged(detectionResult)){const processedMessage=this.handleDetectedPII(message,detectionResult,this.strategy,abort);if(this.strategy==="filter"){continue;}else if(this.strategy==="redact"){if(processedMessage){processedMessages.push(processedMessage);}else {processedMessages.push(message);}continue;}}processedMessages.push(message);}return processedMessages;}catch(error){if(error instanceof TripWire){throw error;}throw new Error(`PII detection failed: ${error instanceof Error?error.stack:"Unknown error"}`);}}/**
...
var ExecutionEngine=class extends chunk4L3P3PCP_cjs.MastraBase{mastra;options;constructor({mastra,options}){super({name:"ExecutionEngine",component:chunkWVCEJBDR_cjs.RegisteredLogger.WORKFLOW});this.mastra=mastra;this.options=options;}__registerMastra(mastra){this.mastra=mastra;}};// src/workflows/step.ts
...
var LegacyWorkflow=class extends chunk4L3P3PCP_cjs.MastraBase{name;triggerSchema;resultSchema;resultMapping;events;#retryConfig;#mastra;#runs=/* @__PURE__ */new Map();isNested=false;#onStepTransition=/* @__PURE__ */new Set();// registers stepIds on `after` calls
...
var SystemPromptScrubber=class{name="system-prompt-scrubber";strategy;customPatterns;includeDetections;instructions;redactionMethod;placeholderText;model;detectionAgent;constructor(options){if(!options.model){throw new Error("SystemPromptScrubber requires a model for detection");}this.strategy=options.strategy||"redact";this.customPatterns=options.customPatterns||[];this.includeDetections=options.includeDetections||false;this.redactionMethod=options.redactionMethod||"mask";this.placeholderText=options.placeholderText||"[SYSTEM_PROMPT]";this.instructions=options.instructions||this.getDefaultInstructions();this.model=options.model;this.detectionAgent=new exports.Agent({name:"system-prompt-detector",model:this.model,instructions:this.instructions,options:{tracingPolicy:{internal:15/* ALL */}}});}/**
...
_agentNetworkAppend=false;/**
   ...
   * @internal
   */async resolveModelConfig(modelConfig,runtimeContext){if(typeof modelConfig==="object"&&"specificationVersion"in modelConfig){return modelConfig;}if(typeof modelConfig==="string"||this.isOpenaiCompatibleObjectConfig(modelConfig)){return new chunkREVAU76X_cjs.ModelRouterLanguageModel(modelConfig);}if(typeof modelConfig===`function`){const fromDynamic=await modelConfig({runtimeContext,mastra:this.#mastra});if(typeof fromDynamic===`string`||this.isOpenaiCompatibleObjectConfig(fromDynamic)){return new chunkREVAU76X_cjs.ModelRouterLanguageModel(fromDynamic);}return fromDynamic;}const mastraError=new chunkWM4VQWOZ_cjs.MastraError({id:"AGENT_GET_MODEL_MISSING_MODEL_INSTANCE",domain:"AGENT"/* AGENT */,category:"USER"/* USER */,details:{agentName:this.name},text:`[Agent:${this.name}] - No model provided`});this.logger.trackException(mastraError);this.logger.error(mastraError.toString());throw mastraError;}/**
...
messages:messageList.get.remembered.v1(),// these are here for inspecting but shouldn't be returned by the processor
// - ex TokenLimiter needs to measure all tokens even though it's only processing remembered messages
newMessages:messageList.get.input.v1(),systemMessage,memorySystemMessage:memorySystemMessage||void 0});const processedList=new chunkX5SB7NR3_cjs.MessageList({threadId:threadObject.id,resourceId,generateMessageId:this.#mastra?.generateId?.bind(this.#mastra),// @ts-ignore Flag for agent network messages
...
//# sourceMappingURL=chunk-ECMIW6W2.cjs.map
//# sourceMappingURL=chunk-ECMIW6W2.cjs.map