...
async function validateAndSaveScore(storage, payload) {
  const payloadToSave = chunkT5SM2HLV_cjs.saveScorePayloadSchema.parse(payload);
...
exports.Mastra = class Mastra {
  ...
  constructor(config) {
    ...
    this.#serverCache = new chunkUU5L5GDY_cjs.InMemoryServerCache();
    if (config?.pubsub) {
    ...
    const workflowEventProcessor = new chunkYICS4NNU_cjs.WorkflowEventProcessor({
      mastra: this
    });
    ...
    if (config?.logger === false) {
      ...
      if (config?.logger) {
        ...
        const levelOnEnv = process.env.NODE_ENV === "production" && process.env.MASTRA_DEV !== "true" ? chunkWVCEJBDR_cjs.LogLevel.WARN : chunkWVCEJBDR_cjs.LogLevel.INFO;
        logger = new chunkWVCEJBDR_cjs.ConsoleLogger({
          name: "Mastra",
          level: levelOnEnv
        });
      }
    ...
    if (config?.observability) {
      chunk7ZXDZXGD_cjs.setupAITracing(config.observability);
    }
...
`
  ...
  registerAITracingExporters() {
    const allTracingInstances = chunk7ZXDZXGD_cjs.getAllAITracing();
   ...
   */
  async shutdown() {
    await chunk7ZXDZXGD_cjs.shutdownAITracingRegistry();
...
//# sourceMappingURL=chunk-PPCSJI73.cjs.map
//# sourceMappingURL=chunk-PPCSJI73.cjs.map