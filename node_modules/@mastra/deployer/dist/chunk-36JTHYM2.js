  ...
  return [
    ...
    bundlerOptions.isDev ? {
      ...
      async resolveId(id, importer, options) {
        ...
        const pkgJson = await getPackageInfo(path2.join(rootDir, info.name));
        if (!pkgJson) {
          return null;
        }
        const pkgName = pkgJson.packageJson.name || "";
        let resolvedPath = resolve.exports(pkgJson.packageJson, id.replace(pkgName, "."))?.[0];
        if (!resolvedPath) {
          resolvedPath = pkgJson.packageJson.main ?? "index.js";
        }
        return await this.resolve(path2.posix.join(pkgJson.rootPath, resolvedPath), importer, options);
      }
    } : null,
    optimizeLodashImports({
      include: "**/*.{js,ts,mjs,cjs}"
...
export { aliasHono, analyzeBundle, collectTransitiveWorkspaceDependencies, esbuild, extractMastraOption, getBundlerOptions, getPackageName, getPackageRootPath, getWorkspaceInformation, packWorkspaceDependencies, removeAllOptionsFromMastraExcept, removeDeployer2 as removeDeployer, slash, tsConfigPaths };
//# sourceMappingURL=chunk-36JTHYM2.js.map
//# sourceMappingURL=chunk-36JTHYM2.js.map