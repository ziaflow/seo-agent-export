    ...
    setRequestHandler(requestSchema, handler) {
        ...
        if (method === 'elicitation/create') {
            const wrappedHandler = async (request, extra) => {
                var _a;
                const validatedRequest = types_js_1.ElicitRequestSchema.safeParse(request);
                if (!validatedRequest.success) {
                    throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Invalid elicitation request: ${validatedRequest.error.message}`);
                }
                const result = await Promise.resolve(handler(request, extra));
                const validationResult = types_js_1.ElicitResultSchema.safeParse(result);
                if (!validationResult.success) {
                    throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Invalid elicitation result: ${validationResult.error.message}`);
                }
