        ...
        this.server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request, extra) => {
            ...
            try {
                ...
                if (tool.inputSchema) {
                    ...
                    const parseResult = await (0, zod_compat_js_1.safeParseAsync)(schemaToParse, request.params.arguments);
                    if (!parseResult.success) {
                        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Input validation error: Invalid arguments for tool ${request.params.name}: ${(0, zod_compat_js_1.getParseErrorMessage)(parseResult.error)}`);
                    }
                ...
                if (tool.outputSchema && !result.isError) {
                    ...
                    if (!parseResult.success) {
                        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Output validation error: Invalid structured content for tool ${request.params.name}: ${(0, zod_compat_js_1.getParseErrorMessage)(parseResult.error)}`);
                    }
                }
            }
            catch (error) {
    ...
    setPromptRequestHandlers() {
        ...
        this.server.setRequestHandler(types_js_1.GetPromptRequestSchema, async (request, extra) => {
            ...
            if (prompt.argsSchema) {
                ...
                if (!parseResult.success) {
                    throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Invalid arguments for prompt ${request.params.name}: ${(0, zod_compat_js_1.getParseErrorMessage)(parseResult.error)}`);
                }
                const args = parseResult.data;
